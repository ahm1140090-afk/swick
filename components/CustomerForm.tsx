...
const CustomerForm: React.FC<CustomerFormProps> = ({ customer, onClose }) => {
  const { addCustomer, updateCustomer, customers, setIsEditing } = useAppContext();
  ...
  useEffect(() => {
    if (customer) {
      setFormData({
        ...
        const lastCustomerNum = customers.reduce((max, c) => {
            const num = parseInt(c.customerNumber?.split('-')[1] || '0', 10);
            return num > max ? num : max;
        }, 0);
        const newCustomerNumber = `CUST-${(lastCustomerNum + 1).toString().padStart(3, '0')}`;
        setFormData(prev => ({ ...prev, customerNumber: newCustomerNumber }));
    }
  }, [customer, customers]);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-lg m-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-gray-800 dark:text-white">{customer ? 'تعديل العميل' : 'إضافة عميل جديد'}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="sm:col-span-1">
                    <label htmlFor="customerNumber" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">رقم العميل</label>
                    <input type="text" name="customerNumber" value={formData.customerNumber} onChange={handleChange} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600" required />
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="tel" name="phone" value={formData.phone} onChange={handleChange} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" />
                </div>
            </div>
            <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                 <div>
                    <label htmlFor="idNumber" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">رقم البطاقة (اختياري)</label>
                 ...
                 <div>
                    <label htmlFor="idExpirationDate" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">تاريخ الانتهاء (اختياري)</label>
                    <input type="date" name="idExpirationDate" value={formData.idExpirationDate} onChange={handleChange} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600" />
                </div>
            </div>
