...
// FIX: Define InvoiceDetailProps interface
...
const InvoiceDetail: React.FC<InvoiceDetailProps> = ({ invoice, onClose }) => {
    const { customers, companyInfo, formatCurrency, paymentReceipts, accounts } = useAppContext();
    ...
    const balanceDue = invoice.totalAmount - (invoice.paidAmount || 0) - (invoice.returnedAmount || 0);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-start z-50 p-4 sm:p-10 overflow-y-auto">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-4xl">
                <div className="p-4 sm:p-6 flex justify-between items-center border-b dark:border-gray-700 no-print">
                    <h2 className="text-xl font-bold text-gray-800 dark:text-white">تفاصيل الفاتورة #{invoice.invoiceNumber}</h2>
                    <div className="flex items-center gap-2">
                         <button onClick={() => window.handlePrint('printable-invoice-detail')} className="bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-500 text-gray-800 dark:text-gray-300 px-4 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors flex items-center">
                ...
                <div id="printable-invoice-detail" ref={invoiceRef} className="p-6 sm:p-10 text-gray-800 dark:text-gray-200">
                    {/* Invoice Header */}
                    <div className="flex justify-between items-start mb-8">
                        ...
                        <h2 className="text-3xl font-bold text-gray-700 dark:text-gray-300 uppercase">فاتورة</h2>
                    </div>
                    ...
                    <div className="grid sm:grid-cols-2 gap-4 mb-8">
                        ...
                        <div className="text-right">
                             ...
                             {invoice.orderNumber && <p><span className="font-semibold">رقم الطلب:</span> {invoice.orderNumber}</p>}
                             <p><span className="font-semibold">نوع الفاتورة:</span> {invoice.isCash ? 'نقدية' : 'آجلة'}</p>
                             <p><span className="font-semibold">تاريخ الإصدار:</span> {invoice.issueDate}</p>
                        </div>
                    </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-right">
                            <thead className="bg-gray-100 dark:bg-gray-700">
                                <tr>
                                    <th className="p-3 font-semibold text-right">البند</th>
                            ...
                            <tbody>
                                {invoice.items.map(item => (
                                    <tr key={item.id} className="border-b dark:border-gray-700">
                                        ...
                                        <td className="p-3 text-left font-mono">{formatCurrency(item.quantity * item.unitPrice)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                                ...
                                <table className="w-full text-sm text-right">
                                    <thead className="bg-gray-100 dark:bg-gray-700">
                                        <tr>
                                            ...
                                            <th className="p-3 font-semibold text-right">حساب الدفع</th>
                    ...
                    <div className="flex justify-end mt-6">
                        <div className="w-full sm:w-2/5">
                            ...
                            {(invoice.returnedAmount || 0) > 0 && (
                                <div className="flex justify-between py-2 text-gray-600 dark:text-gray-300">
                                    <span>المرتجعات</span>
                                    <span className="font-mono text-orange-500">({formatCurrency(invoice.returnedAmount || 0)})</span>
                                </div>
                            )}
                            <div className="flex justify-between py-2 text-gray-600 dark:text-gray-300">
                                <span>المبلغ المدفوع</span>
                                <span className="font-mono text-green-500">({formatCurrency(invoice.paidAmount || 0)})</span>
                            </div>
                    ...
                    {/* Footer */}
                    <div className="mt-12 text-center text-sm text-gray-500 dark:text-gray-400 border-t pt-4 dark:border-gray-700">
